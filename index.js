...
function makeSmartPerson(myName) {
  const myObj = {
    name: myName,
    sum: (a,b) => a + b,
    speak: () => "Hello, my name is ${name}" + myName
  }
  return myObj
}
...
function getCarInfoByIndex(inventory, index) {
  return `This a ${inventory[index].car_make} ${inventory[index].car_model}`
}
...
function getLastCarInfo(inventory) {
  return `This a ${inventory[inventory.length-1].car_make} ${inventory[inventory.length-1].car_model}`
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventory, identity) {

  for (var i=0; i < inventory.length; i++) {
    if (inventory[i].id === identity) {
      return `This a ${inventory[i].car_make} ${inventory[i].car_model}`
    }
  }
